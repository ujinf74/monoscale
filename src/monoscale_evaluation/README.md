...
| --- | ---: | ---: | ---: | ---: |
| **ATE/거리** | **0.0234 %** | **0.0402 %** | **0.1833 %** | **0.2893 %** |
| **끝오차/거리** | **0.0324 %** | **0.0662 %** | **0.1582 %** | **0.2361 %** |
| RPE 5 m | 0.142 % | 0.196 % | | |
| hop% | 0.14 | 0.17 | 0.83 | 1.35 |
| walk | 1.09 | 2.52 | 0.76 | 1.17 |
| 반복폭 | 1.07x | | | |
...
적합된 승수를 들어내고 프레임의 1 mm를 고친 결과, **벤치는 반복폭 안에서 제자리이고
held-out은 ATE −2.5%, 끝오차 −11%**다. 튜닝하지 않은 세트가 좋아진 것이 요점이고,
0.0226이라는 아침 숫자는 벤치의 노면 구성비에 맞춘 상수가 만든 것이었다.

**주의**: 트래커 파라미터(마운트 높이 등)를 바꿀 때는 **추출과 설정 파일을 함께**
바꿔야 한다. 추정기도 같은 설정에서 높이를 읽으므로, 추출만 바꾸면 두 경로가 서로
다른 기하를 쓰게 되고 채점이 조용히 틀린다. 이 문서의 이전 판(0.0257 / 0.1883)이
그 상태에서 측정된 값이다.
