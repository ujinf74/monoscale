...
## 지금 (2026-09-11, 마운트 높이 보정 후)

|  | 벤치 9 평균 | 벤치 9 최악 | held-out 평균 | held-out 최악 |
| --- | ---: | ---: | ---: | ---: |
| **ATE/거리** | **0.0257 %** | **0.0403 %** | **0.1883 %** | **0.2920 %** |
| **끝오차/거리** | **0.0414 %** | **0.0577 %** | **0.1794 %** | **0.2643 %** |
| RPE 5 m | 0.138 % | 0.178 % | | |
| hop% | 0.13 | 0.18 | 0.83 | 1.35 |
| walk | 1.26 | 2.98 | 0.76 | 1.17 |
| 반복폭 | 1.06x | | | |

오늘 세 변경을 거쳤다. 순서대로 무엇이 얼마였는지:

| | 벤치 ATE/거리 | 벤치 끝오차 | held-out ATE | held-out 끝오차 |
| --- | ---: | ---: | ---: | ---: |
| 아침 (`ef10fe7`) | 0.0226 % | 0.0287 % | 0.1880 % | 0.1777 % |
| `road_step_arc` 배포 | 0.0224 | 0.0281 | 0.1875 | 0.1750 |
| `photometric_scale` 제거 | 0.0523 | 0.0919 | 0.1903 | 0.1891 |
| **마운트 높이 보정** | **0.0257** | **0.0414** | **0.1883** | **0.1794** |

`photometric_scale`을 들어낸 값(0.0523%)이 프레임의 1 mm를 고쳐 0.0257%로 돌아왔다.
**적합 상수가 사던 것의 대부분은 프레임 오차였다.** 아침 수치(0.0226%)에 완전히는
닿지 않지만, 그 0.0226은 벤치의 노면 구성비에 맞춰진 상수가 만든 것이고 held-out에는
1.5%밖에 기여하지 않았다. 지금 수치는 양쪽 세트 모두에서 정직하다.
