...
struct Estimator::Solved
{
  ...
  // Where the map says the vehicle is, as a pose and not as a hop.
  //
  // This is the whole of what makes the map different from every other
  // measurement here. A pair solve, a photometric step, an inertial
  // propagation all say how far the vehicle moved; the map says where it is.
  // Turning that into `relative_motion(pose_, placed)` and composing it is
  // arithmetically the same as setting the pose to `placed`, which is an
  // absolute update with no uncertainty at all -- and once it is a hop,
  // nothing downstream can tell it from one, which is why the rules about not
  // mixing them have to be written by hand.
  //
  // Carried here so the update can treat it as what it is.
  ...
  Camera & camera, std::optional<double> yaw_delta, std::optional<double> yaw_guess)
{
  ...
  if (anchored.count() >= settings_.ground_min_inliers) {
    ...
    if (aligned.has_value()) {
      ...
      // camera, and `map_ready_`/`warming_up` are built on that. Clearing it
      // to mean "the hop did not come from the map" pinned all 899 frames of
      // straight120_v2 in warm-up, where every one of them coasted.
  ...
  if (estimate.has_value()) {
    ...
    for (Eigen::Index i = 0; i < paired; ++i) {
      ...
      if (range > settings_.radial_min_range_m) {
        ...
        // One owner for one quantity. `pair_tilt_gain` reads this same
        // regression per solve and takes the correction out of the hop; this
        // reads it over the whole drive and takes it out of the ranges. They
        // are alternatives, not complements, and letting a configuration set
        // both corrects the mean twice.
        //
        // Splitting them by timescale -- the drive-long mean here, the
        // deviation there -- is the obvious repair and it loses, 5.471% against
        // 5.292% over nine KITTI sequences, because the map below is clamped to
        // +-2% and the mean tilt's effect on the hop reaches 4%. Half a
        // correction cannot own the mean.
        if (settings_.pair_scale_gain != 0.0 && settings_.pair_tilt_gain == 0.0 &&
          camera.pair_n > 2000.0)
        {
...
void Estimator::process_pair()
{
  ...
  fold_landmarks(solved, motion, yaw_delta);
